...
- 非公開プロジェクトの読み・書き・検索は `cosense login` の PAT / Service Account
  ...
  (`PLAN-live-edit.md`)。**位置づけ(2026-09-12 に確定)**: cosentty は PAT で
  完結する。sid を足すと他人の編集が即時に届く。あとは非公開プロジェクトの
  テーマ・表示名・アップロード先を `config.toml` に書かずに済む(sid があれば本家の
  設定を写す。書いた値はそれより優先)。それ以外は何も変わらない。
  無ければ 3→60 秒に伸びるポーリング(`R` で即時読み直し)・アップロード先 `gcs`
  に縮退する。図はテキスト描画のみ(ブラウザ画像の経路は同日に撤去)
