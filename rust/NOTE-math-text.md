# NOTE-math-text: 数式の TUI テキスト描画

`code:tex` / `code:latex` のブロックを、端末の文字だけで2次元に組む。
Mermaid のテキスト段(NOTE-mmd-text.md)と同じ骨格を使い、違うのは
**ブラウザ段が無い**ことだけ。

## 方式: `term-maths` に任せる

- 依存: `term-maths = "1.0"`(MIT OR Apache-2.0)。呼ぶのは `render()` のみ
- 依存の依存は `rust-latex-parser` / `unicode-segmentation` / `unicode-width` の3つ
- 自前で持つのは整形・非対応の見分け・幅の見張りだけ
  (`src/bin/view/math_text.rs`)

`mermaid-text` と違い、`RenderedBlock` はセルを `String` で持ち幅を
`unicode-width` で数える。`\frac{速度}{時間}` の全行が同じ表示幅で揃うので、
mmd 側でやった continuation cell の補正は要らない。

## 縮退順(1ブロックあたり)

1. 編集中(edit session がブロック内) → 素のソース(既存契約が勝つ)
2. `COSENSE_MATH=off`(`App::math_text`) → 素のコード行
3. テキスト描画できる式 → `Row::Line`(選択・検索・yank が効く)
4. 描けない → 素のコード行

Mermaid にある「ブラウザ画像」の段が無い。Cosense は KaTeX で数式を描いて
いるが、`#mermaid-preview-<lineId>` のように**要素を実地で確かめていない**。
見当違いの場所を撮るくらいなら、テキストとソースで止める。
`ArtifactKind::web()` が `None` を返すことでその意思を型に載せてある
(`Block::Artifact` は Mermaid と数式で共有する)。

## 入力の整形

`\end{...}` の直前と式の末尾にある `\\` を落とす。行区切りの記号なので、
最後の行に付いていると lib が空の行をもう一段組み、`⎝  ⎠` だけの行が浮く。
Cosense の作例(ヘルプ「数式」)がまさにその書き方なので、素通しにはできない。

## 非対応の見分け

lib は知らない命令をエラーにせず**そのまま吐く**(`\begin{align}` が
`\begin{align}` という字面で出る)。読めない字面を数式のふりで見せるより
コードブロックのほうがましなので、出力にバックスラッシュ＋英字が残って
いたらパース失敗とみなして降りる(`looks_unparsed`)。

`\begin{align}` / `\qquad` は非対応。`pmatrix` / `bmatrix` / `cases` /
`\frac` / `\sqrt` / `\int` / `\sum` / `\lim` / ギリシャ文字 / 上下付きは通る。

## 段: 左端だけ

図は2段まで入れ子にできるが、数式は**0段だけ**(`MATH_MAX_INDENT`)。
箇条書きのインデントが1段でも付いた時点で、コードブロックとしても認識せず、
`code:tex` の行も中身の行もそれぞれ普通のリストの行になる
(mmd の3段目以降と同じ扱い)。境界は `artifact_too_deep` が一箇所で持ち、
`code_span_at` / `code_line_flags` / `render_lines_with` の3つが同じ答えを見る。

## 空行: 打てる。出るのはインデントを削り切ったとき

編集の契約は mmd/コードと共通。ブロックの末尾で `Enter` するとインデント付きの
空行が積める(何個でも)。出るのはその空行のインデントを `⌫` で削り切って
フラッシュになったとき — その行はもう外で、打った文字は普通の行になる。

描画だけは数式らしさがある。LaTeX に空行は意味がなく、`term-maths` に
渡すと空行が組図の**中**に紛れ込んで形を壊す。だから空行は投入前に落とす
(ブロックが生きている間、数式は組まれ続ける)。

## 幅

数式は折り返せない(2次元の組みが崩れる)。ペイン幅に入らなければ描かず、
ソースを見せる。実測ではヘルプ掲載の式はすべて80桁に収まる。

## panic

LaTeX パーサは入力を受ける境界。mmd と同じく `catch_unwind` で囲み、
lib 内の panic を viewer 全体の終了にしない。

## インライン `[$ ... ]`

括弧の中は Cosense の文ではなく LaTeX なので、リンク・装飾・アイコンの
いずれとしても読み取らない(`decorate_bracket` の最初の分岐)。

- **1行で組める式**はその場で置き換える。`[$ E = mc^2 ]` → `E = mc²`、
  `[$ 3 \times 2 ]` → `3 × 2`。1行に何個でも置ける
- **2行以上になる式**(`\frac`・`\sqrt`・`\sum`・`\int`)は行の部品として切り出し、
  画像と同じ仕組みで行の中に置く(下節)

色はコードと同じ。下線を付けない——この viewer では下線が「Enter で追える」
の印になる。
括弧の中はページ名ではないので、`positioned_links_on_line`(フッタの
「Enter で開く」とクリックの当たり判定)も数式を選ばない。

## 複数行のインライン: 画像と同じ箱に乗せる

文字と画像の混ざった行は、既に `Block::Inline` / `layout_inline` としてある。
そこへ `InlinePart::Formula` を足した。

違いは**箱の伸びる向き**だけ。画像なら本文の行の上に立ち、数式なら
本文の行を**またぐ**——分子は上、分母は下。そこで `layout_inline` の箱を
「高さ」から「ベースラインより上に何行(`above`)・下に何行(`below`)か」へ変えた。
画像は `above = h - 1, below = 0` なので従来と同じ結果になる。

数式の各行は**テキストの片として置く**ので、描画・検索・コピーは
周りの本文と同じ経路に乗る。`Row::Inline` に新しい欄は要らない。

```text
                    ────────
              -b ± √b² - 4ac
二次方程式の解は ────────────────である。
                    2a
```

ペインが狭くて式が入らないときは LaTeX に戻す(数式は折り返せないが、
文字列なら本文と同じように折り返せる)。

## 検証ページ

`https://scrapbox.io/my-sandbox/数式テスト`(公式ヘルプ「数式」の
対応範囲そのまま)。

## 色

- **描かれた数式は本文と同じインク**(色を付けない)。1行の `E = mc²` も
  3行の分数も同じ——1行のときだけコード色、という以前の食い違いを直した。
  Cosense の KaTeX も本文と同じ色で組む
- **描けなかった LaTeX はコード色**(`markup.raw.inline.markdown`)。
  組まれていないそれは「記法」であって、`code:` ラベルと同じ種類のもの
- 端末に固定RGBは書かない。本文のインクは端末が持ち、コード色はテーマから来る

## 編集中のライブプレビュー

編集セッションがブロックの中にいる間、ソースの下に**いま打っている内容の組図**を
出す(cosense web と同じ)。打鍵のたびに再組図する(実測 0.4ms)。違いの演出は、
プレビューの全行に付く薄い左罫 `▏` と、先頭の薄字ラベル「プレビュー」——本文と
読み違えさせないため。lib が組めない式・ペイン幅に入らない式は黙る(ソースが
もう見えているので、何かを出す必要がない)。

プレビューは `Row::Aside`(添え物):帯なし、カーソルも選択も乗らない。
読むためのものなので、キャレットの runway の一部にはしない。

インライン `[$ ... ]` にも同じ。行の編集中、行の中の全数式(`inline_formulas` が
読む——レンダラと同じ走査)を**横に並べて**組む(`math::join_beside`。ベースラインを
そろえるので、1行の式はその行に立ち、分数はまたぐ)。コードブロックの中では
出ない —— あそこは記法が効かない約束の場所で、`[$ ]` はただの文字。
