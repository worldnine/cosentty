...
- **2行以上になる式**(`\frac`・`\sqrt`・`\sum`)は受けない。行の高さが
  変わると折り返しも選択もカーソル列も崩れるので、書かれた LaTeX を
  そのまま見せる(括弧と `$` だけ落ちる)
...
Web に一番近いのは、行ごと複数行に展開して周りの本文をベースラインの行に
置くやり方(`RenderedBlock::baseline()` がある)。ただし行を単位に数えている
折り返し・選択・カーソル列に手が入るので、別の段として扱う。
