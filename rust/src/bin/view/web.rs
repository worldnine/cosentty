...
impl App {
    ...
    pub(crate) fn start_web_renders(&mut self, trigger: capability::Trigger) -> bool {
        ...
        for b in &self.blocks {
            ...
            // `image` means every block the browser can draw, including the
            // ones the text tier could have: the reader chose the picture.
            // (Text mode never gets here — `decide` said Nothing above.)
