...
fn edit_session_hides_the_related_section() {
    ...
    assert_eq!(
        app.src_count(),
        2,
        "related rows are unaddressable in the session"
    );
    let end = app
        .rows
        .iter()
    ...
    assert_eq!(
        ...
        app.rows.len() - 1,
        "nothing follows the frame while editing"
    );
...
fn enter_on_empty_bullet_escapes_the_list() {
    ...
    assert_eq!(
        app.lines[2].text, "",
        "spaces removed from the empty bullet"
    );
...
fn enter_replaces_the_range_with_one_break() {
    ...
    assert_eq!(
        texts(&app),
        vec!["t", "", "def"],
        "the range's text left the page"
    );
    let s = app.session.as_ref().unwrap();
    assert_eq!(s.line, 2);
    assert_eq!(s.input.cur, 0);
...
fn the_footer_says_why_the_page_is_not_the_newest_state() {
    ...
    assert!(
        app.hint_text(&[]).starts_with("同期: poll · "),
        "and it leads the footer"
    );

    app.sync_state = capability::SyncState::Live;
    app.ws_pending
        .push_back(commit("c1", "p0", "pid", "someone", vec![]));
...
fn typing_a_bracket_over_a_selection_wraps_it() {
    ...
    assert_eq!(
        s.input.cur,
        "[改善案]".len(),
        "the caret follows the new link"
    );
...
fn a_table_is_written_with_enter_and_tab() {
    ...
    assert!(
        app.table_span_at_line(3).is_some(),
...
fn an_empty_row_plus_enter_leaves_the_table() {
    ...
    assert!(
        app.table_span_at_line(s.line).is_none(),
        "outside the table"
    );
    ...
    assert_eq!(
        app.session.as_ref().unwrap().input.buf,
        " ",
        "still inside the code"
    );
    assert!(app
        .code_span_at_line(app.session.as_ref().unwrap().line)
        .is_some());
}
...
fn a_cell_separator_is_visible_while_editing() {
    let row = " a\tb";
    let span = cosense::render::CodeSpan {
        header: 0,
        header_indent: 0,
    ...
    for byte in [1usize, 2, 3, 4] {
        ...
        assert_eq!(
            raw_caret_from_display(row, d, Some(span)),
            byte,
            "byte {byte}"
        );
    }
...
fn a_pasted_trailing_newline_does_not_leave_a_blank_line() {
    ...
    assert_eq!(
        app.session.as_ref().unwrap().line,
        2,
        "on the last pasted line"
    );
...
fn the_mouse_in_edit_selects_characters_across_lines() {
    ...
    handle_mouse_content(
        &mut app,
        &ctx,
        mouse(MouseEventKind::Down(MouseButton::Left), 8, 3),
    );
    handle_mouse_content(
        &mut app,
        &ctx,
        mouse(MouseEventKind::Up(MouseButton::Left), 8, 3),
    );
    ...
    assert!(
        ...
        "EDIT never selects a line band with the mouse"
    );
    assert_eq!(
        copy_payload(&app, false).unwrap().0,
        "llo world\nsecond line\nth",
...
fn a_drag_that_wobbles_across_lines_stays_characters() {
    ...
    handle_mouse_content(
        &mut app,
        &ctx,
        mouse(MouseEventKind::Drag(MouseButton::Left), 3, 2),
    );
    draw(&mut app);
    let s = app.session.as_ref().unwrap();
    assert_eq!(s.line, 0);
...
fn a_word_is_letters_and_digits_across_scripts() {
    ...
    assert_eq!(
        word_span("hello world", 5),
        Some((0, 5)),
        "right edge still takes it"
    );
    assert_eq!(word_span("hello world", 6), Some((6, 11)));
    assert_eq!(
        word_span("hello  world", 6),
        None,
        "inside the gap: nothing"
    );
    assert_eq!(
        word_span("abc こんにちは", 4),
        Some((4, 19)),
        "a CJK run is a word"
    );
    assert_eq!(
    ...
    assert_eq!(
        word_span("あいx", 5),
        Some((0, 7)),
        "one run of letters: all of it"
    );
    assert_eq!(
        display_caret("あいx", 5, None),
        3,
        "and the caret maps the same way"
    );
}
...
fn enter_on_a_code_header_opens_the_block_body() {
    ...
    handle_session_key(&mut app, &ctx, key(KeyCode::Enter));
    assert_eq!(
        app.lines[1].text, "code:x.py",
        "the header committed with the split"
    );
...
fn enter_on_a_blank_code_line_stays_in_the_block() {
    ...
    assert_eq!(
        app.lines[3].text, " ",
        "the blank code line keeps its indent"
    );
...
fn enter_in_code_keeps_the_line_s_own_depth() {
    ...
    assert_eq!(
        app.lines[4].text, "   ",
        "the new line starts at the same depth"
    );
    let s = app.session.as_ref().unwrap();
    assert_eq!(
        (s.line, s.input.cur),
...
fn a_code_line_shows_no_bullet_while_editing() {
    ...
    let shown: Vec<String> = rows
        ...
        .filter_map(|r| match r {
            Row::Line { line, src, .. } if *src == 2 => Some(
                line.spans
                    .iter()
    ...
    assert_eq!(
        shown, rendered,
        "the caret line sits exactly where the code sits"
    );
    assert_eq!(
        shown,
        vec!["   indented".to_string()],
        "code gutter, no bullet"
    );
...
fn session_paste_multiline_becomes_lines() {
    ...
    assert_eq!(
        app.lines[3].text, "Zb",
        "line tail follows the last fragment"
    );
    let s = app.session.as_ref().unwrap();
    assert_eq!(s.line, 3);
...
fn tab_on_a_plain_code_header_moves_the_whole_block() {
    ...
    assert!(
        app.line_in_code(2) && app.line_in_code(3),
        "still one block"
    );
    let s = app.session.as_ref().unwrap();
...
fn space_at_the_head_of_a_plain_code_header_nests_the_block() {
    ...
    type_str(&mut app, &ctx, " ");
    assert_eq!(
        app.lines[1].text, " code:text.txt",
        "no stray space in the text"
    );
...
fn ctrl_j_breaks_the_line_like_enter() {
    ...
    assert_eq!(
        app.lines[2].text, " |bbbbb",
        "^j is Enter: the indent rides along"
    );
}
...
fn ctrl_s_commits_the_dirty_line_and_says_so() {
    ...
    assert_eq!(
        app.lines[1].text, " before after",
        "the dirty line went out"
    );
