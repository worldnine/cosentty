...
fn scrolled_image_placeholder_reclaims_the_top_body_row() {
    ...
    terminal.draw(|f| ui(f, &mut app, &ctx)).unwrap();
    {
        ...
        assert!(
            rows.iter()
                .any(|r| r.contains("[https://example.com/a.png]")),
            "rows: {rows:?}"
        );
        assert_eq!(
            app.rows
                .iter()
                .find(|r| matches!(r, Row::ImageLoading { .. }))
                .map(|r| r.height()),
            Some(1)
        );
    }
...
fn a_waiting_picture_sweeps_a_band_along_its_url() {
    ...
    let mut draw_at = |app: &mut App, seconds: f32| -> Vec<(String, ratatui::style::Style)> {
        ...
        let y = (0..8)
            ...
            .expect("the notation on screen");
        let x0 = (0..60)
            .find(|x| buf.cell((*x, y)).unwrap().symbol() == "[")
            .unwrap();
        let x1 = (0..60)
            .rev()
            .find(|x| buf.cell((*x, y)).unwrap().symbol() == "]")
    ...
    assert_eq!(
        early.len(),
        late.len(),
        "the same notation; only its brightness moved"
    );
    ...
    for (sym, st) in &early {
        assert!(
            matches!(st.fg, Some(Color::Rgb(..))),
            "{sym} kept a palette colour: {st:?}"
        );
        assert!(
            !st.add_modifier.contains(Modifier::DIM),
            "the band is colour, not DIM"
        );
    }
    ...
    assert!(
        b >= a + 6,
        "the band covered ground between two frames, not a cell or two: {a} → {b}"
    );
    assert!(
        a > 0,
        "and it entered from the head of the row, not the middle"
    );
    assert!(
        b + 3 < early.len(),
        "…and had not fallen off the end yet: {b}/{}",
        early.len()
    );
}
...
fn a_waiting_picture_names_itself_in_the_box_it_reserved() {
    ...
    assert_eq!(
        hits.len(),
        1,
        "the waiting picture speaks once, on its own row: {hits:?}"
    );
    let y = hits[0];
    assert!(
        row(y).contains("本文"),
        "on the line's own baseline, where the words ride: {}",
        row(y)
    );
    let x0 = (0..80)
        .find(|x| buf.cell((*x, y)).unwrap().symbol() == "[")
