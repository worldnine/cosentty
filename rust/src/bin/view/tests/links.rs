...
fn a_diagram_note_never_hides_or_erases_a_commit_or_auth_message() {
    ...
    assert_eq!(
        app.status, "コミットに失敗しました: line 4 — 500",
...
fn quoted_notation_is_not_a_link() {
    ...
    app.goto_src(1);
    assert!(
        app.cursor_line_links().is_empty(),
        "{:?}",
...
fn a_linked_image_is_one_link_to_its_href() {
    ...
    match &links[0] {
        LinkItem::Url { label, url } => {
            ...
            assert!(
                label.contains("photo.png"),
                "labelled by the picture: {label}"
            );
        }
...
fn file_links_are_found_with_their_labels() {
    ...
    assert_eq!(
        files_on_line(&line),
        vec![("260826ニセコ.pdf".to_string(), pdf.to_string())]
    );
    ...
    assert_eq!(
        ...
        vec![
            ...
            (
                "https://b.example/x?y=1".to_string(),
                "https://b.example/x?y=1".to_string()
            ),
        ]
    );
    ...
    assert_eq!(
        ...
        vec![LinkItem::Url {
            label: "図".into(),
            url: format!("https://acme.gyazo.com/{id}")
        }]
    );
...
fn download_path_prefers_the_label_and_never_overwrites() {
    ...
    assert_eq!(
        name(&download_path_in(&dir, "260826ニセコ.pdf", url)),
        "260826ニセコ.pdf"
    );
    // a label without an extension falls back to the URL's file name
    assert_eq!(
        name(&download_path_in(&dir, "資料", url)),
        "6a8e7e5d714feb3f195319dd.pdf"
    );
...
fn page_urls_from_the_command_line_are_parsed() {
    ...
    assert_eq!(
        parse_page_url(&format!(
            "https://scrapbox.io/acme/{enc}#6a8f895000000000000000e1"
        )),
        Some((
            "acme".into(),
            Some("CBTビジネス部案件".into()),
            Some("6a8f895000000000000000e1".into())
        ))
    );
    assert_eq!(
    ...
    assert_eq!(
        parse_page_url("https://cosen.se/p/a%2Fb"),
        Some(("p".into(), Some("a/b".into()), None))
    );
    // titles may contain a slash: everything after the project is title
    assert_eq!(
        parse_page_url("https://scrapbox.io/p/a/b"),
        Some(("p".into(), Some("a/b".into()), None))
    );
    assert_eq!(
        parse_page_url("acme"),
        None,
        "a bare project name is not a URL"
    );
    assert_eq!(parse_page_url("https://example.com/x/y"), None);
    // decoding round-trips the encoder used for browser deep links
    assert_eq!(
        percent_decode(&urlencode_component("階層整理型WiKi はスケールしない")),
        "階層整理型WiKi はスケールしない"
    );
...
fn a_link_inside_a_decoration_is_a_link_to_follow() {
    ...
    assert_eq!(
        links_on_line("[* [改善案]] と [/ [斜体の中]]"),
        vec![page("改善案"), page("斜体の中")]
    );
    assert_eq!(links_on_line("[* ただの太字]"), vec![]);
    // `[[x]]` は太字、`[[[x]]]` は太字のリンク。
    assert_eq!(
        links_on_line("[[太字]] のあと [[[改善案]]]"),
        vec![page("改善案")]
    );
    assert_eq!(
        links_on_line("[[太字]] [普通のリンク]"),
        vec![page("普通のリンク")]
    );
    // 位置は内側の `[` のもの(クリックの当たり判定に使う)。
    let pos: Vec<usize> = positioned_links_on_line("[* [a]] [b]")
        .into_iter()
        .map(|(p, _)| p)
        .collect();
