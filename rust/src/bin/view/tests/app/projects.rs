...
fn the_projects_list_filters_on_slug_and_name_and_offers_nothing_to_create() {
    ...
    let entries: Vec<Entry> = two_projects()
        .into_iter()
        .map(Entry::from_project)
        .collect();
    assert_eq!(entries[0].title, "Acme, Inc.");
    assert_eq!(entries[0].slug, "acme");
    assert_eq!(
        entries[1].title, "my-sandbox",
    ...
    assert!(
        matches!(ix.rows()[..], [Row::Page(e)] if e.slug == "acme"),
        "matched by slug"
    );
    ...
    assert!(
        ix.rows().is_empty(),
        "no create row for a name that is not a project"
    );
    ...
    assert_eq!(
        ix.filter_mode,
        FilterMode::Title,
        "projects have no bodies to search"
    );
}
...
fn control_o_steps_up_to_the_projects_and_enter_steps_back_down() {
    ...
    assert!(
        app.toast_text().contains("更新の新しい順"),
        "{}",
        app.toast_text()
    );
    ...
    assert!(
        matches!(app.forward.last(), Some(Place::Index { state, .. }) if state.scope == Scope::Projects)
    );
}
