...
fn an_indent_only_exit_replaces_the_block_and_keeps_its_ids() {
    ...
    assert!(
        ...
        "and it uses the structural gate"
    );
...
fn one_undo_reverses_a_whole_drag() {
    ...
    assert!(
        ...
        "one press put the whole drag back"
    );
...
fn a_line_step_always_goes_and_the_sibling_step_jumps_a_subtree() {
    ...
    handle_key(
        &mut app,
        &ctx,
        modified(KeyCode::Char('J'), KeyModifiers::SHIFT),
    );
    ...
    handle_key(
        &mut app,
        &ctx,
        modified(KeyCode::Char('K'), KeyModifiers::SHIFT),
    );
...
fn a_lost_block_restores_the_arrangement_from_before_the_grab() {
    ...
    assert!(
        app.toast_text().contains("つかむ前の並び"),
        "status: {}",
        app.toast_text()
    );
}
...
fn click_moves_cursor_and_drag_selects_a_line_range() {
    ...
    handle_mouse_content(
        &mut app,
        &test_ctx(),
        mouse(MouseEventKind::Drag(MouseButton::Left), 12, 8),
    );
    ...
    handle_mouse_content(
        &mut app,
        &test_ctx(),
        mouse(MouseEventKind::Drag(MouseButton::Left), 12, 2),
    );
...
fn tab_cycles_through_link_lines_and_teaches_z_when_there_are_none() {
    ...
    assert!(
        bare.toast_text().contains("ソース表示は z"),
        "note: {}",
        bare.toast_text()
    );
}
...
fn z_toggles_the_raw_source_view_instead_of_tab() {
    ...
    assert!(
        ...
        "the footer badge says SRC; no toast"
    );
    handle_key(&mut app, &ctx, key(KeyCode::Char('z')));
...
fn the_comments_list_deletes_with_d() {
    ...
    assert!(
        matches!(app.overlay, Some(Overlay::Comments { cursor: 0 })),
        "cursor clamped, list still open"
    );
    handle_key(&mut app, &ctx, key(KeyCode::Char('d')));
...
fn s_with_nowhere_to_send_keeps_the_comments_and_says_so() {
    ...
    assert!(
        app.toast_text().contains("送るコメントがありません"),
        "{}",
        app.toast_text()
    );
    app.cursor = 1;
    ...
    assert_eq!(
        app.comments.len(),
        1,
        "nothing was delivered, so nothing is cleared"
    );
    assert!(app.toast.as_ref().unwrap().error);
    assert!(
        app.toast_text().contains("送り先がありません"),
        "{}",
        app.toast_text()
    );
    assert!(
        app.toast_text().contains("コメントは残しています"),
        "{}",
        app.toast_text()
    );
}
...
fn c_on_the_same_range_edits_the_existing_comment() {
    ...
    handle_key(&mut app, &ctx, key(KeyCode::Char('c')));
    assert_eq!(
        app.composing.as_ref().unwrap().buf,
        "typo",
        "prefilled with the existing text"
    );
...
fn the_composer_takes_line_breaks_with_ctrl_j() {
    ...
    handle_key(&mut app, &ctx, key(KeyCode::Down));
    assert_eq!(
        app.composing.as_ref().unwrap().cur,
        5,
        "same column (2) on the next line"
    );
    handle_key(&mut app, &ctx, ctrl('a'));
    assert_eq!(
        app.composing.as_ref().unwrap().cur,
        3,
        "^a: this line's start, not the draft's"
    );
    ...
    assert!(
        app.status.is_empty() || !app.status.contains("^j"),
        "the composer hint is gone"
    );
}
