...
fn splice_puts_the_picture_in_with_breathing_room() {
    ...
    assert_eq!(
        splice_image("a b", 2, "u"),
        ("a [u] b".into(), 4),
        "a space already there is kept"
    );
    assert_eq!(
        splice_image("日本", 1, "u"),
        ("[u] 日本".into(), 4),
        "offset inside a char backs up"
    );
    assert_eq!(
        splice_image("ab", 99, "u"),
        ("ab [u]".into(), 4),
...
fn an_upload_landing_on_the_caret_line_goes_into_the_buffer() {
    ...
    assert_eq!(
        s.input.cur,
        s.input.buf.len() - 1,
        "caret still before the b"
    );
    assert!(
        drain_jobs(&mut app).is_empty(),
        "nothing committed yet: the line is just dirty"
    );
    assert!(
        app.note_text().contains("貼りました"),
        "{}",
        app.note_text()
    );
    assert!(
        !app.hint_body(&[]).contains("アップロード中"),
        "the in-flight note left the footer: {}",
        app.hint_body(&[])
    );
}
...
fn an_upload_for_another_line_replaces_that_lines_current_text() {
    ...
    assert_eq!(
        ...
        "two",
        "the caret line is untouched"
    );
    let jobs = drain_jobs(&mut app);
    assert!(matches!(
        jobs.last().and_then(|(_, ops)| ops.first()),
        Some(EditOp::Replace { .. })
    ));
}
...
fn a_failed_upload_only_reports_and_a_foreign_pages_result_is_dropped() {
    ...
    app.drain_uploads(&ctx);
    assert!(
        app.toast_text().contains("容量"),
        "402 is said in words: {}",
        app.toast_text()
    );
    ...
    assert!(
        !app.drain_uploads(&ctx),
        "a result for the page we left changes nothing"
    );
    assert_eq!(app.lines[1].text, "one");
}
...
fn pasting_an_image_path_starts_an_upload_instead_of_typing_it() {
    ...
    assert_eq!(
        app.session.as_ref().unwrap().input.buf,
        "one",
        "the path is not typed"
    );
    assert!(
        app.status.contains("アップロード中") && app.status.contains("gcs"),
        "{}",
        app.status
    );
    assert!(
        app.status.contains("読めない"),
        "no setting was read, and the status says so: {}",
        app.status
    );
...
fn uploads_are_refused_with_a_reason_when_they_cannot_happen() {
    ...
    app.start_upload(&ctx, &png);
    assert!(
        app.toast_text().contains("読み取り専用"),
        "{}",
        app.toast_text()
    );
}
...
fn a_gyazo_destination_wants_the_matching_token() {
    ...
    app.start_upload(&ctx, &png);
    assert!(
        app.toast_text().contains("GYAZO_ACCESS_TOKEN") && !app.toast_text().contains("TEAMS"),
        "{}",
        app.toast_text()
    );
