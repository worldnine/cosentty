...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if let Some(sid) = &sid {
        ...
        if let Ok(mut hint) = app.ws_page_hint.lock() {
            *hint = (!loaded.page_id.is_empty()).then(|| {
                (
        ...
        cosense::ws::spawn_ws_sync_with_page_id_hint(
            ctx.client.clone(),
            sid.clone(),
            Arc::clone(&app.poll_target),
            ws_req_rx,
...
fn run(
    ...
    // 前フレームの描画バッファ。画像の下に文字が幽霊のように残る行を
    // 見つけるために持つ(images::image_ghost_remains)。
    let mut prev_frame: Option<ratatui::buffer::Buffer> = None;
    loop {
        ...
        let mut frame = terminal.draw(|f| ui(f, app, ctx))?.buffer.clone();
        // 画像の上に被さっていた文字が、覆いが退いても画像の行に残る
        // ことがある(kitty のプレースホルダ行は先頭セルが変わらないと
        // 再描画されない)。見つけたら画面を捨てて全面を描き直す。
        if prev_frame
            .as_ref()
            .is_some_and(|p| image_ghost_remains(p, &frame))
        {
            terminal.clear()?;
            frame = terminal.draw(|f| ui(f, app, ctx))?.buffer.clone();
        }
