...
## code: ブロックの中のリンクを聞いていた

同じページで、リンクの生死を問う HEAD + v1 GET が5組（10本）出ていた。中身を見たら
`code:markdown.md` ブロックに貼った Markdown のサンプルで、`## テスト` から
「`#` というページ」「`##` というページ」を作り出して聞きに行っていた。

`probe_unknown_links` は生の行を走査していて、インラインコード（バッククォート）は
`mask_inline_code` で外していたが、**`code:` ブロックの本文は外していなかった**。
ブロックの中身はレンダラが逐語で描くテキストで、リンクは色も付かない——答えを得ても
描く場所がない質問だった。`render::code_line_flags` はこの判定を既に持っていたので、
走査から外すだけで済む。

実測: **16本 → 6本**（20秒開いたまま。本文1・関連1・プロジェクト情報3・users/me 1）。
