...
## 実施記録: テーマ・表示名も file > API に揃えた(2026-09-12)

上の「API が読めるときは API」は取り下げた。手で書いた値が、API が読める
環境では黙って効かない(設定画面に「API が優先」の注記を出して補っていた)のは、
設定画面の挙動として説明しにくい。sid の位置づけを「ws 受信だけ」に絞る整理と
合わせ、`[project.<slug>]` の3項目を揃って **file > API > 既定** にした
(`Ctx::project_theme_with` / `project_display_with`)。sid を入れている人は今まで
どおり何も書かずに本家の設定が写る。「API が優先」の注記は不要になったので消した。
