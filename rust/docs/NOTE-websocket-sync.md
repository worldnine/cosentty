...
## 追補: 起動時のページ取得（2026-09-11）

起動時は、初期表示の `page-v2` に加えて、WebSocket worker がjoin前のページID解決と
join後のcatch-upで同じページを取得していた。join前の取得はページIDだけが目的で、応答本文を
画面へ適用していない。

初期表示で得た永続ページのIDをWebSocket workerへ一度だけ渡し、join前のID解決に再利用する。
catch-upは残す。初期表示とjoinの間に行われた編集を取りこぼさないため、catch-upを省略しては
ならない。ページ移動・再接続時は従来どおりjoin前の取得を行う。これにより通常の起動時の
`page-v2` は3回から2回になる（初期表示1回 + join後catch-up 1回）。

実機でSIDあり・12秒の起動を計測し、`page-v2` は起動直後と約3.96秒後の2回だった。
