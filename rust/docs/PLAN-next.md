...
- 2026-09-12: **1ページの表示にかかる要求を数える仕掛けを入れ、重複を削った**。
  `COSENSE_HTTP_LOG=<path>` で全要求を1行ずつ記録できる。実測では help-jp の1ページ表示で
  scrapbox.io へ**11本**飛んでいた（本文v2 / 関連v1 / スナップショット /
  projectId / ws の join 前本文 / ws の catch-up / users/me / projects ×2 …）。
  projectId のキャッシュ、`/projects/<p>/users` と `/projects/<p>` の
  単発化（同時に走る2スレッドが同じ答えを2回取りに行っていた）、ws の join 前取得の廃止
  （直前のページ設置が pageId を知っているので `ws::PageIdHint` で渡す）、
  履歴APIの403の記憶、`page_exists` の60秒メモで **9本 → 移動時は4〜5本**になった。
  あわせて、429で関連ページ取得が落ちたときに**リンク1本ずつの問い合わせへ縮退していた**
  経路（429が429を呼ぶ）を止め、連続429ではクールダウンを倍々に広げる。
  正本は `NOTE-rate-limit.md`。続けて **(a) ページ移動直後の3秒ポーリング（ws が live ならやめる）**
  と **(c) 毎ページの履歴件数取得（← で入ったときに取る）** も切った。
  **(b) ws の join 後 catch-up も廃止した**。ページ応答の `commitId` が次の commit
  イベントの `parent` そのものだと実サーバで確かめた上で（`ws_smoke` が検査する）、
  手元のページから連鎖を続ける。欠落は「親が繋がらない」ことで検出され、そのときだけ
  再同期する。引き換えは「窓の間に1件 commit があり、その後誰も書かない」場合の
  最大60秒（保険ポーリング）遅延。

  最後に、その243行ページで HEAD+v1 が5組（10本）飛んでいた件は
  **`code:` ブロックの中身を走査していたバグ**だった。貼ってあった Markdown サンプルの
  `## テスト` からタグ `#` ・`##` を作り出して聞きに行っていた。ブロックの本文は
  逐語描画でリンクの色も付かないので、答えを得ても描く場所がない。
  `render::code_line_flags` で走査から外して **16本 → 6本**（20秒開いたまま。
  本文1・関連1・プロジェクト情報の初回のみ4）。

  調べたが**やらないと決めたこと**: `/api/pages/<project>/search/titles` は全ページの
  タイトルとリンクを一括で返す（1万件ずつ、`X-Following-Id` で続き。実測で
  villagepump 45,119ページ = 3.0MB×5本、1.8秒/本）ので、原理上はリンク問い合わせを
  ゼロにできる。が、上のバグを直したあとに残るのは「セッション中に打った新しい
  リンク」だけで、大きいプロジェクトで14MB取る割に合わない。必要になったらここを読むこと。
- 2026-09-12: **sid の位置づけを「ws 受信を速くするだけ」に絞った**(作者判断。
  sid はアカウント全権のブラウザ cookie で、PAT のようなスコープも失効管理も無い。
  土台にはせず、無しでも困らないようにする)。同時編集の頻度は「頻繁とも
  たまにとも言えない」ので ws 自体は残す。やったこと: (1) headless Chrome による
  図の画像描画(`diagrams = image`)を撤去(`NOTE-mmd-text.md` の実施記録。後方互換なし)、
  (2) プロジェクトのテーマ・表示名を画像の保存先と同じ **file > API > 既定** に
  揃えた(`PLAN-settings-ui.md` の実施記録)、(3) 空いた `R` を**手動リロード**に
  (ポーリングが60秒まで伸びる代わりの逃げ道)、(4) README・HANDOFF・KEYMAP の
  記述を「PAT で完結。sid を足すと他人の編集が即時に届き、非公開の設定を書かずに済む。
  それ以外は変わらない」に揃えた。sid の参照は `App::has_sid` と ws の接続だけ。
- 2026-09-12: **テストのオフライン疑似を DNS 非依存にした**。`offline_ctx` は `.invalid`
  の名前解決失敗に頼っていて、リゾルバの遅い網では数十秒待って結果が揺れていた
  (`only_a_failed_structural_save_holds_the_jobs_behind_it` が通ったり落ちたり)。
  loopback の ephemeral ポートを bind して即 drop し、そこへ接続拒否させる。
  失敗が即時になって露わになった同テストの競合(gen を進めるのが worker の取り出しより
  先だと j3 が Skipped になる)も、j3・j4 の結果を待ってから進める順序に直した。
  bin のテスト全体が 6秒 → 1秒台。
- 2026-09-11: **起動時429を致命エラーにしない**。一覧取得429では一覧を再試行できるTUIを
  起動し、ページ取得429では読み取り専用の仮ページからバックグラウンド再取得する。
  429以外の起動エラーは従来どおり終了する。詳細は `NOTE-rate-limit.md`。
- 2026-09-11: **起動時の同一ページ取得を3回から2回へ削減**。初期表示で得た永続ページのIDを
  WebSocketのjoin前解決に一度だけ再利用し、join後のcatch-upは残す。詳細は
  `NOTE-websocket-sync.md` の追補が正本。
- 2026-09-11: **429応答後のorigin単位クールダウンを共有**。`Retry-After`または既存の
  指数バックオフをプロセス内の後続要求にも適用し、並列ワーカーの再送集中を抑える。
  詳細は `NOTE-rate-limit.md`。
- 2026-09-11: **SID が無いときの固定3秒ポーリングを、未変更なら
  3→6→12→30→60秒へ伸ばす方式に変更**(`NOTE-websocket-sync.md` の追補が正本)。
  変更の検出・ページ移動・端末からの保存で3秒へ戻す。実測では1プロセスのアイドル時
  要求を20件/分から1件/分へ減らせる
