...
- 2026-09-12: **1ページの表示にかかる要求を数える仕掛けを入れ、重複を削った**。
  ...
  **次に大きいのはリンク生死の問い合わせ**。sandbox の243行ページを開くと、
  本文1本・関連2本に対して HEAD+v1 が5組（10本）飛ぶ。`#` や `##` のような
  ページ応答の `links` に現れないリンクが毎回未知になるため。HEAD の方は60秒メモが
  効くが、backlink 側（v1 GET）は毎回出る。未着手。
