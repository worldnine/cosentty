...
- 2026-09-12: **1ページの表示にかかる要求を数える仕掛けを入れ、重複を削った**。
  ...
  残る重複は **(b) ws の join 後 catch-up**（毎移動で本文をもう一度）だけ。
  代替案は「取得済みページの `commitId` を `ws_head` の種にする」——欠落は
  次の commit の親が繋がらないことで検出できるので、先回りの再取得をやめられる。
  ただし「窓の間に1件だけ commit があり、その後誰も書かない」場合は最大
  60秒（保険ポーリング）遅れる。未着手。
