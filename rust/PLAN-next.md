...
- 一覧の操作を続けると **429 Too Many Requests** に当たる(2026-09-03 実測。
  並び順の切り替えは毎回 500 件を取り直し、本文検索も1回ずつ叩く)。
  当面は API 呼び出しが 429 を `Retry-After` だけ待って言い直す(`SendPolite`)
  のと、画像の同時ダウンロードを4本に絞るので凌ぐ。本格対策は並び順ごとの
  一覧をセッション内でキャッシュすること
...
- 右端は状態だけ: **書かれた日時**(NOW は最新行の `updated`、履歴は快照の時刻 + `n/N`。
  ...
  幅が足りないときは右側を優先し、左の名称/タイトルを `…` で削る。履歴中はヘッダと枠を
...
黄バナー(拒否と案内、他人の編集)/ 赤バナー(失敗。8秒)。`web_notice` は「薄く」に
