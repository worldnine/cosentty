...
全体像とドキュメント索引は `HANDOFF.md`、次のタスクは `rust/docs/PLAN-next.md`。

## ビルド・テスト

- `cd rust && cargo test --bin cosentty` が基本の検証。lib 含む全部は `cargo test`
...
- **同じ target dir を共有するとバイナリは残らない**。worktree と本体が
  `target/debug/cosentty` を上書きし合い、`cargo build` は再リンクを省くことがある。
...
- viewer は `rust/src/bin/view/` 配下に関心事別で分割済み。
  ...
  詳しい対応表は `rust/docs/NOTE-codebase-review.md` を参照する。
...
- 変更は worktree を切って行い、テスト green を確認してから main へ
...
- 実験・検証は自分の非公開プロジェクト(無料で作れる)の `テスト` ページで行う
  ...
  読み取り(テーマ・アップロード先)だけ
- 設計判断は `rust/docs/` の PLAN-*.md / NOTE-*.md / SPEC-*.md に書き残す文化。実施した
  計画には「実施記録」を追記する
